...
@synthesize bufferStorageArray, soundDictionary, bufferDictionary, hardwareConnected;
...
- (void) setHardwareConnected:(BOOL)isHardwareConnected {
    if(isHardwareConnected) {
        printf("connecting hardware");
...
-(void)loadFile:(NSString *)soundName withKey:(NSString*)key doesLoop:(BOOL)loops
{
    ...
    // store the bufferID for future use
    [bufferDictionary setObject:[NSNumber numberWithUnsignedInt:bufferID] forKey:key];
...
/* -(void) playSound:(NSString*)soundKey withLoops:(int)loops {
    NSNumber * numVal = [soundDictionary objectForKey:soundKey];
	if (numVal == nil)
	{
		NSLog(@"sound doesnt exist!");
    ...
    NSUInteger bufferID = [[bufferDictionary objectForKey:soundKey] unsignedIntValue];
    alSourcei(sourceID, AL_LOOPING, AL_FALSE);
    for(int i = 0; i < loops; i++) {
        // TODO THIS LINE NOT WORKING:
        alSourceQueueBuffers(sourceID, AL_BUFFER, &bufferID);
    }
	alSourcePlay(sourceID);
} */
